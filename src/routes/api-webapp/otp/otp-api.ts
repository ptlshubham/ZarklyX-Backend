...
router.post("/send-otp", async (req: Request, res: Response): Promise<any>=> {
  try {
    ...
    // OTP update payload in OTP table
    if (email) {
      // For email-based OTP, we can use `otp`
      otpRecord.otp = String(otp);
      otpRecord.otpExpiresAt = expiresAt;
      otpRecord.otpVerify = false;
    }
    if (contact) {
      // For mobile-based OTP, we can use `mbOTP`
      otpRecord.mbOTP = String(otp);
      otpRecord.mbOTPExpiresAt = expiresAt;
      otpRecord.otpVerify = false;
    }
...
router.post("/resend-otp", async (req: Request, res: Response): Promise<any> => {
  ...
  try {
    ...
    let otpRecord = await Otp.findOne({
      where: { userId: user.id }, // 🔁 numeric id
      transaction: t,
    });

    if (!otpRecord) {
      // If not exists, create new OTP record
      otpRecord = await Otp.create(
        {
          userId: user.id,
          email: user.email || email || null,
    ...
    if (!sendResult || !sendResult.success) {
      await t.rollback();
      return serverError(res, sendResult?.message || "Failed to resend OTP.");
    }
